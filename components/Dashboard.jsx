...
export default function App(){
  ...
  const doSyncWithConfig=async(cfg)=>{
    ...
    try{const sync=await syncAll(url,token);
      ...
      log(`Sync retornou: ${sync.stats?.success||0}/${sync.stats?.total||0} endpoints`,"ok");
      if(sync.report){
        Object.entries(sync.report).forEach(([k,v])=>{
          log(`${k} (${v.endpoint||"?"}): ${v.success?v.count+" registros":"FALHOU"+(v.error?" — "+v.error:"")}`,v.success?"ok":"error");
        });
      }
      // Log raw data keys and sizes
      if(sync.data){
        const sizes=Object.entries(sync.data).map(([k,v])=>{
          if(Array.isArray(v))return `${k}:${v.length}`;
          if(v&&v.data&&Array.isArray(v.data))return `${k}:${v.data.length}`;
          if(v&&v.paginacao)return `${k}:${v.paginacao.quantidadeItens||"?"}items`;
          return `${k}:${typeof v}`;
        });
        log("Raw data: "+sizes.join(", "),"info");
      }
      const dashboard=buildDashboard(sync.data||{});
      log(`Dashboard: ${dashboard.produtos?.length||0} prod, ${dashboard.fornecedores?.length||0} forn, R$${(dashboard.kpis?.receitaBruta||0).toFixed(2)} vendas`,"ok");
