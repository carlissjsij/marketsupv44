...
let SETORES=[{n:"Mercearia",vd:823400,cp:612000,mg:28.5,pt:28.3,rcv:74.3,mv:850000,mm:27,sk:1420},{n:"Açougue",vd:521300,cp:498000,mg:24.7,pt:17.9,rcv:95.5,mv:500000,mm:25,sk:85},{n:"FLV",vd:312500,cp:198000,mg:38.2,pt:10.7,rcv:63.4,mv:300000,mm:35,sk:210},{n:"Padaria",vd:289100,cp:142000,mg:52.1,pt:9.9,rcv:49.1,mv:280000,mm:48,sk:95},{n:"Bebidas",vd:445600,cp:389000,mg:22.3,pt:15.3,rcv:87.3,mv:460000,mm:23,sk:680},{n:"Frios/Latic.",vd:298400,cp:215000,mg:31.4,pt:10.3,rcv:72.1,mv:310000,mm:30,sk:340},{n:"Limpeza",vd:142300,cp:108000,mg:26.8,pt:4.9,rcv:75.9,mv:150000,mm:28,sk:380},{n:"Higiene",vd:77800,cp:58000,mg:29.1,pt:2.7,rcv:74.5,mv:80000,mm:27,sk:290}];

let FORN=[{n:"Camil Alimentos",cp:189400,pr:34,lt:3,cd:"28/42 DDL",uc:"25/02"},{n:"Frigorífico JBS",cp:312000,pr:18,lt:1,cd:"7/14 DDL",uc:"27/02"},{n:"Ambev",cp:145600,pr:42,lt:2,cd:"21 DDL",uc:"26/02"},{n:"Coca-Cola FEMSA",cp:98700,pr:28,lt:2,cd:"21 DDL",uc:"24/02"},{n:"Bunge Alimentos",cp:78900,pr:12,lt:4,cd:"28 DDL",uc:"22/02"},{n:"CEASA Regional",cp:67800,pr:95,lt:0,cd:"À Vista",uc:"27/02"},{n:"Kraft Heinz",cp:45200,pr:22,lt:5,cd:"42 DDL",uc:"20/02"},{n:"Unilever",cp:89300,pr:65,lt:3,cd:"28/42 DDL",uc:"23/02"}];

let VD7=[{d:"Seg",v:412300,m:420000},{d:"Ter",v:389500,m:420000},{d:"Qua",v:445200,m:420000},{d:"Qui",v:478100,m:420000},{d:"Sex",v:523400,m:480000},{d:"Sáb",v:612800,m:560000},{d:"Dom",v:298100,m:320000}];
...
export default function App(){
  ...
  const doSyncWithConfig=async(cfg)=>{
    ...
    try{const sync=await syncAll(url,token);
      if(!sync.success){setApiState(p=>({...p,syncing:false,error:sync.error}));log("FALHA sync: "+sync.error,"error");return;}
      log(`Sync retornou: ${sync.stats?.success||0}/${sync.stats?.total||0} endpoints em ${sync.elapsed||"?"}ms`,"ok");
      if(sync.report){
        Object.entries(sync.report).forEach(([k,v])=>{
          log(`${k}: ${v.success?v.count+" registros":"FALHOU"+(v.error?" — "+v.error:"")}`,v.success?"ok":"error");
        });
      }
      // Use pre-built dashboard from server
      const dashboard=sync.dashboard;
      if(dashboard){
        log(`Dashboard: ${dashboard.produtos?.length||0} prod, ${dashboard.fornecedores?.length||0} forn, R$${(dashboard.kpis?.receitaBruta||0).toFixed(2)} vendas, R$${(dashboard.kpis?.totalCompra||0).toFixed(2)} compras`,"ok");
        // Map server field names to frontend format
        const mappedProdutos=(dashboard.produtos||[]).map(p=>({
          id:p.id, nome:p.n, grupo:p.g, marca:p.m, curva:p.c,
          custo:p.cu, preco:p.pr, mg:p.mg, est:p.e,
          forn:p.f, unidade:p.u, vd:p.vd||0, qtd:p.qt||0, lucro:p.lu||0,
          meta_mg:p.c==="A"?22:p.c==="B"?28:p.c==="C"?32:25, ativo:true,
        }));
        const mappedForn=(dashboard.fornecedores||[]).map(f=>({
          id:f.id, n:f.n, cp:f.cp, uc:f.uc, pr:f.pr, cnpj:f.cn||"", lt:3, cd:"—",
        }));
        setRealData({
        ...
        log("ERRO: Servidor não retornou dashboard","error");
      }
      setRealData(prev=>{const d=prev||{};return d;});
  ...
  // Override demo data with real data when connected
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        {apiState.connected&&(
          <div style={{padding:"6px 24px",background:D.greenBgD,borderBottom:`1px solid ${D.greenSoft}20`,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
            ...
            <div style={{flex:1}}/>
            {realData?.kpis&&<span style={{fontSize:9,color:D.greenSoft,fontFamily:FM}}>{realData.kpis.totalProdutos||0} prod | R$ {((realData.kpis.receitaBruta||0)/1000).toFixed(0)}k vendas</span>}
